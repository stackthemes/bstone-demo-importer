...
require_once BDIM_ABSPATH . 'includes/functions-demo-update.php';
...
function tg_ajax_install_required_plugin() {
	...
	if ( empty( $_POST['plugin'] ) || empty( $_POST['slug'] ) ) {
		wp_send_json_error(
			array(
				...
				'errorMessage' => __( 'No plugin specified.', 'bstone-demo-importer' ),
			)
		);
	}
	...
	if ( ! current_user_can( 'install_plugins' ) ) {
		$status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'bstone-demo-importer' );
 ...
 */
if ( apply_filters( 'bstone_clear_data_before_demo_import', true ) ) {
	add_action( 'bstone_ajax_before_demo_import', 'tg_reset_widgets', 10 );
	add_action( 'bstone_ajax_before_demo_import', 'tg_delete_nav_menus', 20 );
	add_action( 'bstone_ajax_before_demo_import', 'tg_remove_theme_mods', 30 );
}
...
if ( class_exists( 'WooCommerce' ) ) {
	add_action( 'bstone_ajax_demo_imported', 'tg_set_wc_pages' );
}
				...
				<# if ( data.successes ) { #>
					<# if ( 1 === data.successes ) { #>
						<# if ( 'plugin' === data.type ) { #>
							...
							printf( __( '%s plugin successfully installed.', 'bstone-demo-importer' ), '{{ data.successes }}' );
							?>
						<# } #>
					<# } else { #>
						<# if ( 'plugin' === data.type ) { #>
							<?php
							/* translators: %s: Number of plugins */
							printf( __( '%s plugins successfully installed.', 'bstone-demo-importer' ), '{{ data.successes }}' );
							?>
						<# } #>
					<# } #>
				<# } #>
				<# if ( data.errors ) { #>
					<button class="button-link bulk-action-errors-collapsed" aria-expanded="false">
						<# if ( 1 === data.errors ) { #>
							...
							printf( __( '%s install failed.', 'bstone-demo-importer' ), '{{ data.errors }}' );
							?>
						<# } else { #>
							<?php
							/* translators: %s: Number of failed installs */
							printf( __( '%s installs failed.', 'bstone-demo-importer' ), '{{ data.errors }}' );
							?>
						<# } #>
						<span class="screen-reader-text"><?php _e( 'Show more details', 'bstone-demo-importer' ); ?></span>
