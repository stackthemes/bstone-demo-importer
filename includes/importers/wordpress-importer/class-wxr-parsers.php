...
class TG_WXR_Parser {
	function parse( $file ) {
		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . __( 'There was an error when reading this WXR file', 'bstone-demo-importer' ) . '</strong><br />';
			echo __( 'Details are shown above. The importer will now try again with a different parser...', 'bstone-demo-importer' ) . '</p>';
		}
...
class TG_WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'bstone-demo-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version )
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'bstone-demo-importer' ) );
