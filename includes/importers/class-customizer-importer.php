...
class TG_Customizer_Importer {
	...
	public static function import( $import_file, $demo_id, $demo_data ) {
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error( 'bstone_customizer_import_data_error', __( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'bstone-demo-importer' ) );
		}

		if ( ! empty( $demo_data['template'] ) && ! in_array( $data['template'], $demo_data['template'], true ) ) {
			return new WP_Error( 'bstone_customizer_import_wrong_theme', __( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'bstone-demo-importer' ) );
		}

		// Import Images.
		if ( apply_filters( 'bstone_customizer_import_images', true ) ) {
			$data['mods'] = self::import_customizer_images( $data['mods'] );
		}

		// Modify settings array.
		$data = apply_filters( 'bstone_customizer_demo_import_settings', $data, $demo_data, $demo_id );
