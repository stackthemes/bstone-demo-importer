 ...
 *
 * @package Bstone_Demo_Importer\Classes
 * @version 1.0.0
 */
...
class TG_Demo_Importer {
	...
	public function __construct() {
		...
		if ( apply_filters( 'bstone_show_demo_importer_page', true ) ) {
		...
		// Help Tabs.
		if ( apply_filters( 'bstone_demo_importer_enable_admin_help_tab', true ) ) {
		...
		add_action( 'bstone_ajax_demo_imported', array( $this, 'update_nav_menu_items' ) );
		add_action( 'bstone_ajax_demo_imported', array( $this, 'update_elementor_data' ), 10, 2 );
		add_action( 'bstone_ajax_demo_imported', array( $this, 'update_siteorigin_data' ), 10, 2 );

		// Update widget and customizer demo import settings data.
		add_filter( 'bstone_widget_demo_import_settings', array( $this, 'update_widget_data' ), 10, 4 );
		add_filter( 'bstone_customizer_demo_import_settings', array( $this, 'update_customizer_data' ), 10, 2 );
	}
	...
	private function get_demo_packages() {
		$packages = get_transient( 'bstone_demo_importer_packages' );
		$template = strtolower( str_replace( '-pro', '', get_option( 'template' ) ) );

		if ( false === $packages || ( isset( $packages->slug ) && $template !== $packages->slug ) ) {
			
			$raw_packages = wp_safe_remote_get( "https://wpbstone.com/wp-json/wp/v3/" );

			if ( ! is_wp_error( $raw_packages ) ) {
				$packages = json_decode( wp_remote_retrieve_body( $raw_packages ) );

				if ( $packages ) {
					set_transient( 'bstone_demo_importer_packages', $packages, WEEK_IN_SECONDS );
				}
			}
		}

		return apply_filters( 'bstone_demo_importer_packages_' . $template, $packages );
	}
	...
	private function get_import_file_path( $filename ) {
		return trailingslashit( BDIM_DEMO_DIR . '/dummy-data' ) . sanitize_file_name( $filename );
	}
	...
	public function admin_menu() {
		add_theme_page( __( 'Demo Importer', 'bstone-demo-importer' ), __( 'Demo Importer', 'bstone-demo-importer' ), 'switch_themes', 'demo-importer', array( $this, 'demo_importer' ) );
	}
	...
	public function add_menu_classes() {
		...
		if ( isset( $submenu['themes.php'] ) ) {
			...
			if ( apply_filters( 'bstone_demo_importer_include_class_in_menu', true ) ) {
				foreach ( $submenu['themes.php'] as $order => $menu_item ) {
					if ( 0 === strpos( $menu_item[0], _x( 'Demo Importer', 'Admin menu name', 'bstone-demo-importer' ) ) ) {
	...
	public function enqueue_scripts() {
		...
		$assets_path = bdim()->plugin_url() . '/assets/';

		// Register admin styles.
		wp_register_style( 'tg-demo-importer', $assets_path . 'css/demo-importer.css', array(), BDIM_VERSION );
		...
		wp_register_script( 'tg-demo-updates', $assets_path . 'js/admin/demo-updates' . $suffix . '.js', array( 'jquery', 'updates' ), BDIM_VERSION, true );
		wp_register_script( 'tg-demo-importer', $assets_path . 'js/admin/demo-importer' . $suffix . '.js', array( 'jquery', 'jquery-tiptip', 'wp-backbone', 'wp-a11y', 'tg-demo-updates' ), BDIM_VERSION, true );		
		
		wp_register_script( 'tg_demo_sync', $assets_path . 'js/admin/demo-sync.js', array( 'jquery' ), BDIM_VERSION, true );
		...
		if ( 'appearance_page_demo-importer' === $screen_id ) {
			...
			wp_enqueue_script( 'tg_demo_sync' );
			
			wp_localize_script( 'tg_demo_sync', 'sync_demos', array(
			...
			wp_localize_script(
				...
				array(
					'l10n' => array(
						'importing'             => __( 'Importing...', 'bstone-demo-importer' ),
						'demoImportingLabel'    => _x( 'Importing %s...', 'demo', 'bstone-demo-importer' ), // no ellipsis
						'importingMsg'          => __( 'Importing... please wait.', 'bstone-demo-importer' ),
						'importedMsg'           => __( 'Import completed successfully.', 'bstone-demo-importer' ),
						'importFailedShort'     => __( 'Import Failed!', 'bstone-demo-importer' ),
						'importFailed'          => __( 'Import failed: %s', 'bstone-demo-importer' ),
						'demoImportedLabel'     => _x( '%s imported!', 'demo', 'bstone-demo-importer' ),
						'demoImportFailedLabel' => _x( '%s import failed', 'demo', 'bstone-demo-importer' ),
						'livePreview'           => __( 'Live Preview', 'bstone-demo-importer' ),
						'livePreviewLabel'      => _x( 'Live Preview %s', 'demo', 'bstone-demo-importer' ),
						'imported'              => __( 'Imported!', 'bstone-demo-importer' ),
						'statusTextLink'        => '<a href="https://docs.bstone.com/knowledgebase/demo-import-process-failed/" target="_blank">' . __( 'Try this solution!', 'bstone-demo-importer' ) . '</a>',
					),
				)
			);
			wp_localize_script(
				...
				array(
					...
					'settings' => array(
						...
						'suggestURI'    => apply_filters( 'bstone_demo_importer_suggest_new', 'https://bstone.com/contact/' ),
						'confirmReset'  => __( 'It is strongly recommended that you backup your database before proceeding. Are you sure you wish to run the reset wizard now?', 'bstone-demo-importer' ),
						'confirmImport' => __( "Importing demo data will ensure that your site will look similar as theme demo. It makes you easy to modify the content instead of creating them from scratch. Also consider before importing theme demo: \n\n1. It will install the required plugin as well as activate them for installing theme demo. \n\n2. You need to import demo on fresh WordPress install to exactly replicate the theme demo. \n\n3. None of the posts, pages, attachments or any other data already existing in your site will be deleted or modified. \n\n4. Copyright images will get replaced with other placeholder images. \n\n5. It will take some time to import the theme demo.", 'bstone-demo-importer' ),
					),
					'l10n'     => array(
						'search'              => __( 'Search Demos', 'bstone-demo-importer' ),
						'searchPlaceholder'   => __( 'Search demos...', 'bstone-demo-importer' ), // placeholder (no ellipsis)
						/* translators: %s: support forums URL */
						'error'               => sprintf( __( 'An unexpected error occurred. Something may be wrong with Bstone demo server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.', 'bstone-demo-importer' ), 'https://wordpress.org/support/plugin/bstone-demo-importer' ),
						'tryAgain'            => __( 'Try Again', 'bstone-demo-importer' ),
						'suggestNew'          => __( 'Please suggest us!', 'bstone-demo-importer' ),
						'demosFound'          => __( 'Number of Demos found: %d', 'bstone-demo-importer' ),
						'noDemosFound'        => __( 'No demos found. Try a different search.', 'bstone-demo-importer' ),
						'collapseSidebar'     => __( 'Collapse Sidebar', 'bstone-demo-importer' ),
						'expandSidebar'       => __( 'Expand Sidebar', 'bstone-demo-importer' ),
						/* translators: accessibility text */
						'selectFeatureFilter' => __( 'Select one or more Demo features to filter by', 'bstone-demo-importer' ),
					),
				)
			);
		}
	...
	public function admin_footer_text( $footer_text ) {
		...
		// Check to make sure we're on a Bstone Demo Importer admin page.
		if ( isset( $current_screen->id ) && apply_filters( 'bstone_demo_importer_display_admin_footer_text', in_array( $current_screen->id, array( 'appearance_page_demo-importer' ) ) ) ) {
			// Change the footer text.
			if ( ! get_option( 'bstone_demo_importer_admin_footer_text_rated' ) ) {
				$footer_text = sprintf(
					/* translators: 1: Bstone Demo Importer 2: five stars */
					__( 'If you like %1$s please leave us a %2$s rating. A huge thanks in advance!', 'bstone-demo-importer' ),
					sprintf( '<strong>%s</strong>', esc_html__( 'Bstone Demo Importer', 'bstone-demo-importer' ) ),
					'<a href="https://wordpress.org/support/plugin/bstone-demo-importer/reviews?rate=5#new-post" target="_blank" class="bstone-demo-importer-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'bstone-demo-importer' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
			} else {
				$footer_text = __( 'Thank you for importing with Bstone Demo Importer.', 'bstone-demo-importer' );
			}
		}
	...
	public function add_help_tabs() {
		...
		$screen->add_help_tab(
			array(
				'id'      => 'bstone_demo_importer_support_tab',
				'title'   => __( 'Help &amp; Support', 'bstone-demo-importer' ),
				'content' =>
					'<h2>' . __( 'Help &amp; Support', 'bstone-demo-importer' ) . '</h2>' .
				'<p>' . sprintf(
					__( 'Should you need help understanding, using, or extending Bstone Demo Importer, <a href="%s">please read our documentation</a>. You will find all kinds of resources including snippets, tutorials and much more.', 'bstone-demo-importer' ),
					'https://wpbstone.com/docs/how-to-import-bstone-demo-content/'
				) . '</p>' .
				'<p>' . sprintf(
					__( 'For further assistance with Bstone Demo Importer core you can use the <a href="%1$s">community forum</a>. If you need help with premium themes sold by Stack Themes, please <a href="%2$s">contact our support</a>.', 'bstone-demo-importer' ),
					'https://wordpress.org/support/plugin/bstone-demo-importer',
					'https://stackthemes.net/contact-us/'
				) . '</p>' .
					'<p><a href="' . 'https://wordpress.org/support/plugin/bstone-demo-importer' . '" class="button button-primary">' . __( 'Community forum', 'bstone-demo-importer' ) . '</a> <a href="' . 'https://wpbstone.com/contact-us/' . '" class="button">' . __( 'Bstone Support', 'bstone-demo-importer' ) . '</a></p>',
			)
		);

		$screen->add_help_tab(
			array(
				'id'      => 'bstone_demo_importer_reset_tab',
				'title'   => __( 'Reset wizard', 'bstone-demo-importer' ),
				'content' =>
					'<h2>' . __( 'Reset wizard', 'bstone-demo-importer' ) . '</h2>' .
					'<p>' . __( 'If you need to reset the WordPress back to default again, please click on the button below.', 'bstone-demo-importer' ) . '</p>' .
					'<p><a href="' . esc_url( add_query_arg( 'do_reset_wordpress', 'true', admin_url( 'themes.php?page=demo-importer' ) ) ) . '" class="button button-primary bstone-reset-wordpress">' . __( 'Reset wizard', 'bstone-demo-importer' ) . '</a></p>',
			)
		);

		$screen->set_help_sidebar(
			'<p><strong>' . __( 'For more information:', 'bstone-demo-importer' ) . '</strong></p>' .
			'<p><a href="' . 'https://wpbstone.com/docs/how-to-import-bstone-demo-content/' . '" target="_blank">' . __( 'About Demo Importer', 'bstone-demo-importer' ) . '</a></p>' .
			'<p><a href="' . 'https://wordpress.org/plugins/bstone-demo-importer/' . '" target="_blank">' . __( 'WordPress.org project', 'bstone-demo-importer' ) . '</a></p>' .
			'<p>&nbsp;</p>'
		);
	}
	...
	public function reset_wizard_notice() {
		$screen              = get_current_screen();
		$demo_activated_id   = get_option( 'bstone_demo_importer_activated_id' );
		$demo_notice_dismiss = get_option( 'bstone_demo_importer_reset_notice' );
	...
	public function hide_reset_notice() {
		if ( isset( $_GET['bstone-demo-importer-hide-notice'] ) && isset( $_GET['_bstone_demo_importer_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_bstone_demo_importer_notice_nonce'], 'bstone_demo_importer_hide_notice_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'bstone-demo-importer' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'bstone-demo-importer' ) );
			}

			$hide_notice = sanitize_text_field( $_GET['bstone-demo-importer-hide-notice'] );

			if ( ! empty( $hide_notice ) && 'reset_notice' == $hide_notice ) {
				update_option( 'bstone_demo_importer_reset_notice', 1 );
			}
		}
	}
	...
	public function reset_wizard_actions() {
		...
		if ( ! empty( $_GET['do_reset_wordpress'] ) ) {
			...
			$footer_rated = get_option( 'bstone_demo_importer_admin_footer_text_rated' );
			...
			// Update footer text.
			if ( $footer_rated ) {
				update_option( 'bstone_demo_importer_admin_footer_text_rated', $footer_rated );
			}
			...
			$required_plugins = (array) apply_filters( 'bstone_demo_importer_' . $template . '_required_plugins', array() );
			if ( is_array( $required_plugins ) ) {
				if ( ! in_array( BDIM_PLUGIN_BASENAME, $required_plugins ) ) {
					$required_plugins = array_merge( $required_plugins, array( BDIM_PLUGIN_BASENAME ) );
				}
	...
	public function demo_importer() {
		if( defined( 'BSTONE_PRO_DEMOS' ) && defined( 'BSTONE_PRO_DIR' ) ) {
			include_once BSTONE_PRO_DIR . '/pro-demos/html-admin-page-importer.php';
		} else {
	...
	public function ajax_query_demos( $return = true ) {
		 ...
		 */
		$prepared_demos = (array) apply_filters( 'bstone_demo_importer_pre_prepare_demos_for_js', array(), $available_packages, $demo_activated_id );
		...
		if ( isset( $available_packages->demos ) ) {
			foreach ( $available_packages->demos as $package_slug => $package_data ) {
				...
				$screenshot_url = "https://raw.githubusercontent.com/stackthemes/bstone-demo-pack/master/resources/{$available_packages->slug}/{$package_slug}/screenshot.png";
				...
				$prepared_demos[ $package_slug ] = array(
					...
					'theme'           => $is_pro_theme_demo ? sprintf( esc_html__( '%s Pro', 'bstone-demo-importer' ), $available_packages->name ) : $available_packages->name,
					'isPro'           => $is_pro_theme_demo ? false : isset( $package_data->isPro ),
					'active'          => $package_slug === $demo_activated_id,
					'author'          => isset( $package_data->author ) ? $package_data->author : __( 'Bstone', 'bstone-demo-importer' ),
		 ...
		 */
		$prepared_demos = apply_filters( 'bstone_demo_importer_prepare_demos_for_js', $prepared_demos );
	...
	public function ajax_import_demo() {
		...
		if ( empty( $_POST['slug'] ) ) {
			wp_send_json_error(
				array(
					...
					'errorMessage' => __( 'No demo specified.', 'bstone-demo-importer' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'import' ) ) {
			$status['errorMessage'] = __( 'Sorry, you are not allowed to import content.', 'bstone-demo-importer' );
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			...
			$serializedData = serialize($skin->get_upgrade_messages());
		}
		...
		if ( is_wp_error( $result ) ) {
			...
			$status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'bstone-demo-importer' );
		...
		do_action( 'bstone_ajax_before_demo_import' );
		...
		if ( ! empty( $demo_data ) ) {
			...
			do_action( 'bstone_ajax_demo_imported', $slug, $demo_data );
		}
	...
	public function ajax_footer_text_rated() {
		...
		update_option( 'bstone_demo_importer_admin_footer_text_rated', 1 );
		wp_die();
	}
	...
	public function import_dummy_xml( $demo_id, $demo_data, $status ) {
		...
		do_action( 'bstone_ajax_before_dummy_xml_import', $demo_data, $demo_id );
		...
		if ( is_file( $import_file ) ) {
			...
			do_action( 'bstone_ajax_dummy_xml_imported', $demo_data, $demo_id );

			flush_rewrite_rules();
		} else {
			$status['errorMessage'] = __( 'The XML file dummy content is missing.', 'bstone-demo-importer' );
			wp_send_json_error( $status );
		}
	...
	public function import_customizer_data( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The DAT file customizer data is missing.', 'bstone-demo-importer' );
			wp_send_json_error( $status );
		}
	...
	public function import_widget_settings( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The WIE file widget content is missing.', 'bstone-demo-importer' );
			wp_send_json_error( $status );
		}
				...
				if ( ! empty( $menu_items ) ) {
					foreach ( $menu_items as $menu_item ) {
						if ( isset( $menu_item->url ) && isset( $menu_item->db_id ) && 'custom' == $menu_item->type ) {
							...
							if ( isset( $menu_parts['path'] ) && isset( $menu_parts['host'] ) && apply_filters( 'bstone_demo_importer_nav_menu_item_url_hosts', in_array( $menu_parts['host'], array( 'demo.bstone.com' ) ) ) ) {
